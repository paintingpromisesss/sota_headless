...
**Headless Sota Connect subscription provider**
...
This tool authenticates with their API and serves the nodes as a standard subscription.
...
Your device (Happ / Mihomo / v2rayNG / Zashboard / ...)
        ...
        ▼
  sota-headless  (background service)
        │
...
| **Plain vless://** | `http://<host>:16698/sub/vless` | Manual import |
| **sing-box JSON** | `http://<host>:16698/sub/singbox` | sing-box outbounds array |

> `<host>` — the device's LAN IP address, or `127.0.0.1` if the client runs on the same machine.
