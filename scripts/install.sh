...
main() {
    ...
    if [ "$LANG_UI" = "ru" ]; then
        ...
        [ "$INIT_SYSTEM" = "systemd" ] && printf "    %b↑ IP в локальной сети (может меняться при DHCP)%b\n" "$DIM" "$NC"
        [ "$INIT_SYSTEM" = "systemd" ] && printf "  %bhttp://127.0.0.1:16698/sub/mihomo%b\n" "$DIM" "$NC"
        [ "$INIT_SYSTEM" = "systemd" ] && printf "    %b↑ localhost — только с этого устройства, всегда стабилен%b\n" "$DIM" "$NC"
        printf "\n  %bSing-box outbounds%b\n" "$YEL" "$NC"
        printf "  %bhttp://${ROUTER_IP}:16698/sub/singbox%b\n" "$DIM" "$NC"
        [ "$INIT_SYSTEM" = "systemd" ] && printf "  %bhttp://127.0.0.1:16698/sub/singbox%b\n" "$DIM" "$NC"
        printf "\n  %bBase64 (v2rayN, Nekoray, v2rayNG)%b\n" "$YEL" "$NC"
        printf "  %bhttp://${ROUTER_IP}:16698/sub/base64%b\n" "$DIM" "$NC"
        [ "$INIT_SYSTEM" = "systemd" ] && printf "  %bhttp://127.0.0.1:16698/sub/base64%b\n" "$DIM" "$NC"
        printf "\n  %bПрямые ссылки vless://%b\n" "$YEL" "$NC"
        printf "  %bhttp://${ROUTER_IP}:16698/sub/vless%b\n" "$DIM" "$NC"
        [ "$INIT_SYSTEM" = "systemd" ] && printf "  %bhttp://127.0.0.1:16698/sub/vless%b\n" "$DIM" "$NC"
        printf "\n"
        ...
        printf "  %bMihomo / Clash / Zashboard%b\n" "$YEL" "$NC"
        printf "  %bhttp://${ROUTER_IP}:16698/sub/mihomo%b\n" "$DIM" "$NC"
        [ "$INIT_SYSTEM" = "systemd" ] && printf "    %b↑ LAN IP (may change on DHCP renewal)%b\n" "$DIM" "$NC"
        [ "$INIT_SYSTEM" = "systemd" ] && printf "  %bhttp://127.0.0.1:16698/sub/mihomo%b\n" "$DIM" "$NC"
        [ "$INIT_SYSTEM" = "systemd" ] && printf "    %b↑ localhost — this device only, always stable%b\n" "$DIM" "$NC"
        printf "\n  %bSing-box outbounds%b\n" "$YEL" "$NC"
        printf "  %bhttp://${ROUTER_IP}:16698/sub/singbox%b\n" "$DIM" "$NC"
        [ "$INIT_SYSTEM" = "systemd" ] && printf "  %bhttp://127.0.0.1:16698/sub/singbox%b\n" "$DIM" "$NC"
        printf "\n  %bBase64 (v2rayN, Nekoray, v2rayNG)%b\n" "$YEL" "$NC"
